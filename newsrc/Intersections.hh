...
// template<typename CoordType>
// void
// forceBoundaryCoordinate(Point2<CoordType>& p,
//                         const shapes::BoxSide side) {
//   const auto& Q = Quantizer<2>::instance();
//   if (side == shapes::BoxSide::L) {
//     p.x = Q.minBound.x;
//   } else if (side == shapes::BoxSide::R) {
//     p.x = Q.maxBound.x;
//   } else if (side == shapes::BoxSide::B) {
//     p.y = Q.minBound.y;
//   } else if (side == shapes::BoxSide::T) {
//     p.y = Q.maxBound.y;
//   }
// }
...
template<typename CoordType>
bool
clipParametricLineToBox(const Point2<double>& origin,
                        const Point2<double>& direction,
                        double& t0,
                        double& t1,
                        shapes::BoxSide& enterSide,
                        shapes::BoxSide& exitSide) {
  const auto& Q = Quantizer<2>::instance();
  ...
  auto clipLower =
    [&](const double x,
        ...
        const double lo,
        const shapes::BoxSide side) {
      if (dx == 0.0) return x >= lo;
      const double t = (lo - x)/dx;
      if (dx > 0.0) {
        if (t > t0) {
          t0 = t;
          enterSide = side;
  ...
  auto clipUpper =
    [&](const double x,
        ...
        const double hi,
        const shapes::BoxSide side) {
      if (dx == 0.0) return x <= hi;
      const double t = (hi - x)/dx;
      if (dx > 0.0) {
        if (t < t1) {
          t1 = t;
          exitSide = side;
  ...
  return (clipLower(origin.x, direction.x, Q.rminBound.x, shapes::BoxSide::L) &&
          clipUpper(origin.x, direction.x, Q.rmaxBound.x, shapes::BoxSide::R) &&
          clipLower(origin.y, direction.y, Q.rminBound.y, shapes::BoxSide::B) &&
          clipUpper(origin.y, direction.y, Q.rmaxBound.y, shapes::BoxSide::T));
}

template<typename CoordType>
bool
clipRayToBox(const Point2<double>& origin,
             const Point2<double>& direction,
             Point2<double>& hit,
             shapes::BoxSide& hitSide) {
  double t0 = 0.0;
  double t1 = std::numeric_limits<double>::infinity();

  shapes::BoxSide enterSide = shapes::BoxSide::L;
  shapes::BoxSide exitSide = shapes::BoxSide::L;

  if (!clipParametricLineToBox<CoordType>(origin, direction, t0, t1, enterSide, exitSide)) {
    return false;
  }

  const auto& Q = Quantizer<2>::instance();
  const bool originInside = Q.inQBounds(origin);

  const double tHit = originInside ? t1 : t0;
  hitSide = originInside ? exitSide : enterSide;
  hit = origin + direction*tHit;
...
template<typename CoordType>
bool
clipFiniteSegmentToBox(const Point2<double>& a,
                       const Point2<double>& b,
                       Point2<double>& clippedA,
                       Point2<double>& clippedB,
                       shapes::BoxSide& firstSide,
                       shapes::BoxSide& secondSide,
                       bool& clippedFirst,
                       bool& clippedSecond) {
  const Point2<double> direction = b - a;

  double t0 = 0.0;
  double t1 = 1.0;

  if (!clipParametricLineToBox<CoordType>(a, direction, t0, t1, firstSide, secondSide)) {
    return false;
  }

  clippedA = a + direction*t0;
  clippedB = a + direction*t1;

  clippedFirst = (t0 > 0.0);
  clippedSecond = (t1 < 1.0);
