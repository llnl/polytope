...
void testOverlappingCubes(const int tnum) {
  ...
  // Test intersection
  bool intersects = convexIntersection(qplcA.m_points, qplcA.facets,
                                       qplcB.m_points, qplcB.facets);
...
void testSymmetry(const int tnum) {
  ...
  for (size_t i = 0; i < testPairs.size(); ++i) {
    ...
    bool AB = convexIntersection(qplcA.m_points, qplcA.facets,
                                 qplcB.m_points, qplcB.facets);
    bool BA = convexIntersection(qplcB.m_points, qplcB.facets,
                                 qplcA.m_points, qplcA.facets);
