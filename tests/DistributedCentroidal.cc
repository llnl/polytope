...
vector<double> conditionNumberNodes(Tessellation<2,double>& mesh) {
   ...
   for (unsigned icell = 0; icell != mesh.cells.size(); ++icell) {
      for (unsigned f = 0; f != mesh.cells[icell].size(); ++f) {
         ...
         POLY_CHECK(iface1 < mesh.faces.size() and iface2 < mesh.faces.size());
         POLY_CHECK(mesh.faces[iface1].size() == 2 and mesh.faces[iface2].size() == 2);
         POLY_CHECK(iface1 != iface2);
         ...
         POLY_CHECK(inode2 < mesh.nodes.size()/2);
         POLY_CHECK(inode1 != inode2);
         POLY_CHECK(inode2 != inode3);
         POLY_CHECK(inode1 != inode3);
...
vector<double> conditionNumber(Tessellation<2,double>& mesh) {
  ...
  for (unsigned icell = 0; icell != mesh.cells.size(); ++icell) {
    ...
    for (vector<int>::const_iterator itr = mesh.cells[icell].begin();
      ...
      POLY_CHECK(iface < mesh.faces.size());
      ...
      POLY_CHECK(inode1 != inode2);
      POLY_CHECK(inode1 < mesh.nodes.size()/2 and inode2 < mesh.nodes.size()/2);
